...
def update_corrosion_and_graph(y):
    updated_data = calculate_corrosion(Y=y)
    corrosion_map_updated = updated_data.set_index('SegmentID')['CorrosionLevel'].to_dict()

    # Merge the updated corrosion data into city and service line tables
    city_pipe_main_updated = pd.merge(city_pipe_main, updated_data, on='SegmentID', how='left')
    service_line_table_updated = pd.merge(service_line_table, updated_data, on='SegmentID', how='left')

    # Filter rows with valid coordinates (Lat, Long)
    city_pipe_main_valid_updated = city_pipe_main_updated.dropna(subset=['Latitude', 'Longitude'])
    service_line_table_valid_updated = service_line_table_updated.dropna(subset=['Latitude', 'Longitude'])

    # Rebuild the map figure with updated corrosion levels
    fig = go.Figure()

    # Update city edges and nodes
    city_edges_updated, city_positions_updated = [], {}
    for _, row in city_pipe_main_valid_updated.iterrows():
        city_positions_updated[row['SegmentID']] = (row['Longitude'], row['Latitude'])
        corrosion_level = row['CorrosionLevel']
        corrosion_color = corrosion_colors.get(corrosion_level, 'gray')
        if pd.notna(row['Parent Pipe']):
            parent_row = city_pipe_main_valid_updated[city_pipe_main_valid_updated['SegmentID'] == row['Parent Pipe']]
            if not parent_row.empty:
                parent_lat, parent_lon = parent_row.iloc[0]['Latitude'], parent_row.iloc[0]['Longitude']
                city_edges_updated.append(((parent_lon, parent_lat), (row['Longitude'], row['Latitude']), corrosion_color))
    
    # Add updated edges and nodes to the map
    for edge in city_edges_updated:
        fig.add_trace(go.Scattermapbox(
            lon=[edge[0][0], edge[1][0]],
            lat=[edge[0][1], edge[1][1]],
            mode='lines',
            line=dict(color=edge[2], width=5),
            showlegend=False
        ))

    # Add updated city nodes with corrosion levels
    for segment_id, (lon, lat) in city_positions_updated.items():
        corrosion_level = corrosion_map_updated.get(segment_id, 0)
        corrosion_color = corrosion_colors.get(corrosion_level, 'gray')
        fig.add_trace(go.Scattermapbox(
            lon=[lon],
            lat=[lat],
            mode='markers',
            marker=dict(size=8, color=corrosion_color),
            text=f"SegmentID: {segment_id}, Corrosion Level: {corrosion_level}",
            hoverinfo='text'
        ))

    return f"Slider value (Y): {y}", fig
